...
- **The sidebar adapts to your hardware.** The control panel auto-hides rows you can’t use yet: Lightgun until a Sinden driver is installed, X‑Arcade until a cabinet is identified, RetroArch until RetroArch itself is installed. The **Sidebar** page lets you reorder every entry and show/hide any of them (Auto / Always show / Always hide), with an Apply that updates the sidebar at once.
...
- **Wii Remote navigation**: an optional bridge lets you steer ES‑DE and MAD with a Wii Remote (via a DolphinBar); toggle it under **Main Menu → Input Device Settings → "Wii Remote Navigation"**, and it steps aside automatically whenever a game launches.
...
A full-screen dashboard inside ES-DE, built around a gamepad. It deliberately ignores the keyboard, because a Sinden lightgun types fake keystrokes when it moves, and letting those keystrokes drive the menu would make it wander on its own. A touch mode also works if you turn it on (Main Menu, then Input Device Settings, then the ENABLE TOUCH switch; off by default), so you can tap the screen instead of using a pad; the keyboard stays ignored either way. A sidebar down the left holds the sections; the **L/R shoulder buttons** flip between them; the help strip at the bottom always shows what each button does right now. Changes **save the instant you make them**, and each page re‑reads from disk so what you see is always the truth. You can even navigate it with a Wii Remote (through a DolphinBar).
...
- **Device pins** (this page used to be called "Players"): lock one **exact physical controller** to a player number by pressing a button on it ("press‑to‑identify", you never need to know any technical IDs).
...
- **Standalones**: a tile per standalone emulator you have games for: Sega Model 2, Sega Model 3, Wii/GameCube (Dolphin), Wii U (Cemu), Switch (a group covering Eden, Ryujinx and Citron), PlayStation 3, PlayStation 2, Namco 246/256, Xbox, OpenBOR, LaserDisc games (Daphne), Sega Lindbergh, and M.U.G.E.N. Opening a tile gets you that emulator's **Settings** (video/audio), **Controllers** (which pads are players, with a Hands‑off switch), and where supported **per‑button input mapping** and **per‑game settings**, all on screen, no Desktop Mode. A few tiles (Wii is the main one) also carry the "require a DolphinBar", "require a Sinden gun" and X‑Arcade-warning switches that used to live on a separate "Systems" page, since retired; those flags now live right on the tile of the emulator they affect. The **Namco 246/256** tile (the *pcsx2x6* Namco System 246/256 fork: Tekken, Soul Calibur, Time Crisis, Vampire Night, …) has the full set of sections like the others: **Settings** (graphics, the JVS **Testmode** in-game gun-calibration toggle), **Input mapping** (DualShock2 ports for pad games, plus two USB ports where you pick None / HID Mouse / Light Gun and bind the gun's trigger, pedal, start and coins), a **Lightgun** page when a USB port is Light Gun (crosshair image and size, the Sinden white-border overlay, and a one-press **Start Sinden guns** button), and **Controllers** (assign a gamepad such as a DualSense to a player, for the pad games like Tekken or alongside the Sinden gun in the lightgun games; it persists across launches). *pcsx2x6* is [our Steam‑Deck fork](https://github.com/mmadalone/pcsx2x6/tree/deck-patches); its lightgun mode is single‑player. You install it and add the `pcsx2x6` system yourself, and supply the arcade BIOS plus per‑game dongle/disc (copyrighted; dump from hardware you own, not distributed here). MAD just adds the on‑screen settings + gun‑launch glue.
- **RetroArch**: this row only shows up if RetroArch is actually installed, and opens a hub of several tiles rather than one page. System, Video and Audio hold the global RetroArch settings; Input covers global button and hotkey mapping plus named controller profiles; **Per-system** (this used to be a separate "Priority" page) is where you set, per system, which kind of controller should be Player 1, Player 2 and so on, plus a collection rule (for example, lightgun games) that overrides it; **Bezels** (this used to be its own "Bezel Project" row) turns the decorative screen borders on or off per system or per game, and downloads bezel packs; and Per-game lets you override any of this for one game. For the consoles RetroArch launches, the "warn me if the wrong controller is connected" switch sits right on that same Per-system page (open the system, the switch is there alongside its controller order) rather than on the emulator's own tile.
...
- **On-the-go**: cross-cutting handheld power and resolution profiles. There is one master switch, a choice of how MAD decides you are handheld (automatically, by which screen is active, or you can force handheld or docked), and a default power cap; then a per-system list (PS2, PS3, GameCube, Wii, N64, Saturn, Dreamcast and other demanding systems) where you can set a tighter power cap and scale the rendering resolution down, so a demanding game does not drain the battery or overheat the Deck when you undock and take it with you. It applies across every emulator it covers, not just one.
- **Backup**: a grid of tiles rather than one page. Six tiles for backing up one thing at a time come first, smallest scope first: **Games** (your ROMs, plus the non-Steam shortcut games in your Steam library, including Lutris games), **Emulator config**, **ES-DE settings**, **BIOS**, **System**, and **Controller config**. Then **Full backup**, a single whole-setup archive you can keep on the Deck or push to your MEGA cloud account, and **Manage backups**, where you browse and permanently delete old backup sets (local or cloud). **Restore** comes last: it is its own hub with a tile for each of the six categories above, so you go there deliberately rather than stumbling into it. A **Transfers** tile appears only while a backup or restore is actually running, so you can watch its progress or cancel it.
- **Post-update**: reapply the system setup that a SteamOS update reset (file sharing, lightgun drivers, device rules and the rest), from right inside the panel, no Desktop Mode needed. It asks for your password on screen and shows what it is doing as it runs. If it keeps failing, it tells you the same terminal command as a fallback (see section 7).
- **Sidebar**: **reorder every entry and show/hide each one.** Move a row (press **A** to lift it, up/down to move, **A** to drop), press **X** to cycle a row **Auto / Always show / Always hide**, then **Apply**; the sidebar updates **immediately** and the layout is remembered (no need to reopen the panel). Capability rows (**Lightgun / X‑Arcade / RetroArch**) still **Auto**-hide until their hardware or data is present (handy: if an install‑time Sinden download failed, force Lightgun on to reach its INSTALL button). Core pages can be hidden too, but the **Sidebar** entry itself can be reordered and never hidden, so you can always get back.
...
2. Launch ES‑DE from Steam, open **Main Menu → Utilities → MAD CONTROL PANEL**, and identify your controllers on the **Device pins** page (this page used to be called "Players").
...
- **Wii‑Remote menu navigation.** With the DolphinBar in **mode 4** and at least one remote paired, the patched ES‑DE automatically starts a small bridge so you can drive the whole menu (and the MAD panel) with the remote, nothing to launch by hand. Turn it on or off any time at **Main Menu → Input Device Settings → "Wii Remote Navigation"** (on by default; the switch starts/stops the bridge right away, no restart).
...
- **After a SteamOS *system* update:** SteamOS keeps all your personal files but resets the system core, wiping a few low‑level pieces (file sharing, lightgun deps, device rules, the `input` group, sleep mode). **ES‑DE nudges you on screen** if it notices an update happened, and the easiest fix is right there in the control panel: open **Main Menu → Utilities → MAD CONTROL PANEL → Post-update** and run it from the panel itself, no Desktop Mode needed (it asks for your password on screen). If that keeps failing, fall back to the same script from a Desktop-Mode terminal:
  ```bash
  ~/Emulation/tools/launchers/deck-post-update.sh
  ```
  Either way it reinstalls exactly those pieces (and re‑downloads our ES‑DE if it went missing).
- **After an EmuDeck/ES‑DE *app* update:** that can replace our ES‑DE with stock. The same `deck-post-update.sh` (or the panel's Post-update page) points the launch wrapper back at our build, no rebuild.
- **Local backups:** `deck-backup.sh` archives your whole setup (with choices for the big stuff like ROMs/media); `deck-restore.sh` brings it back on a wiped or new Deck, guiding you and protecting current files during the restore. *(BIOS is backed up by default; the big stuff, ROMs and downloaded media, is off by default but offered, to keep routine backups small.)* The control panel's **Backup** page (see 4a above) covers the same ground on screen, plus finer-grained per-category and per-game backup and restore.
- **Cloud backups (MEGA):** optional and separate from the local backups above. Connect once by getting an access-key file from your MEGA account and dropping it in place, then pressing "CONNECT TO MEGA" on the panel's Full backup tile; the panel walks you through the exact steps. Once connected, small precious things (saves, ES-DE and emulator config, BIOS, MAD's own controller config) can push to MEGA automatically when a game exits, and the big, easily re-downloaded stuff (ROMs, media, cores, bezels) syncs on request with a "Sync library now" button. The same per-category and per-game backup and restore from the local Backup page can also target the cloud, and it covers the non-Steam shortcut games in your Steam library too, including Lutris games. Every cloud push or pull shows up in the **Transfers** tile so you can watch it or cancel it, and it automatically pauses while you are playing a game so it never competes for disk or network with the game itself.
...
**Bezels**
- The easy way is the **Bezels** tile inside the control panel's RetroArch section (it downloads a system's pack on demand). `wire-bezels.py [--apply]` is the bulk command‑line equivalent for wiring RetroArch bezel overlays across many games at once.
...
- **A backup of your ES-DE settings contains your ScreenScraper password**, in plain text, because that is where ES-DE itself stores it (`~/ES-DE/settings/es_settings.xml`). This is a deliberate, accepted design choice, not a bug: MAD just backs up the file exactly as ES-DE writes it. It only matters if you plan to hand an ES-DE settings backup to someone else; think about that before you share one.
- **A restore never overwrites anything in place.** Whatever a restore would replace is moved aside first, into a dated backup folder with its own RECOVERY.txt explaining how to undo it, and only then is the restored copy written. If a restore is ever interrupted partway through, nothing you had is lost; you can always find it in that dated folder.
- **Cloud transfers pause while you play**, so a backup or restore never competes with a running game for disk or network. Anything that would overwrite your live saves or config, a restore or a download that feeds one, stays paused no matter what "Backup during gameplay" says; that toggle only ever lets uploads keep running. A restore you start from the control panel is different again: it runs as part of the panel, so it stops if you close the panel. Let it finish before you back out to play.
...
*This guide describes the project as of August 2026. The technical reference for developers is in `README.md`; cached deep‑dive notes are in `deck-docs/`.*
