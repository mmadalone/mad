...
| **Native MAD panel shell** — `GuiMadPanel` + sidebar + per-section pages under `es-app/src/guis/mad/`, driven by the `mad-backend.py` daemon on [`main`](https://github.com/mmadalone/mad/tree/main) | Runs the control panel as ES-DE GUI components instead of a second X11 window (fixes the gamescope focus split) |
| **THEME FONTS** — UI Settings entry listing the active theme's bundled fonts; overrides the default menu / dialog / MAD font (incl. the `LIGHT`/`BOLD` weights) | Themeable UI font without editing the binary |
| **In-app updater + version label** — `ApplicationUpdater` checks the fork's own rolling `latest-steamdeck` release (compares the CI run number); the main-menu version shows `-MAD.<n>`; the wrapper exports `APPIMAGE` so the in-place update targets the AppImage, not the extracted AppDir | Self-update to fork builds with a visible build number |
