...
DO_ESDE=1; DO_EMU=1; DO_ROMS=0; DO_MEDIA=0
INCLUDE_CORES=1; INCLUDE_BEZELS=1
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --no-cores)   INCLUDE_CORES=0; shift ;;
        --no-bezels)  INCLUDE_BEZELS=0; shift ;;
...
if [[ $ASSUME_YES -eq 0 && $SIZES_ONLY -eq 0 ]]; then
    ...
    ask "Back up standalone emulator settings (~40G: RA, storage, bezels, ikemen)?" Y; DO_EMU=$REPLY_BOOL
...
need_kb=0
for p in "${REAL_ITEMS[@]}"; do need_kb=$(( need_kb + $(du -sk "$p" 2>/dev/null | cut -f1) )); done
