...
# The fan helper's two ROOT-owned files are generated by temp-deck.py, never archived (a
# restored NOPASSWD sudoers rule would silently re-grant privilege). The $HOME marker IS
# restored, so say so only when the opt-in exists but the helper does not.
[[ -f $HOME/.config/temp-deck/fan-helper-installed && ! -e /var/lib/deck-fan/deck-fan-ctl ]] && \
    log "[ ] temp-deck fan control was enabled but its helper is absent — run ~/bin/temp-deck.py --install-fan-helper"
