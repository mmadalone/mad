...
void ApplicationUpdater::compareVersions()
{
    ...
    for (auto& releaseType : releaseTypes) {
        ...
        // The feed is hand-generated by our CI: a malformed releaseNum must
        // not throw on the updater thread (uncaught = abort at startup).
#if !defined(MAD_RELEASE_NUMBER)
#define MAD_RELEASE_NUMBER 0
#endif
        if (releaseType->releaseNum.find_first_not_of("0123456789") !=
            std::string::npos) {
            LOG(LogWarning) << "ApplicationUpdater: non-numeric release number \""
