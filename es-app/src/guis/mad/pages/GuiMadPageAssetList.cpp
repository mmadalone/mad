...
void GuiMadPageAssetList::updateDetail()
{
    if (mDetail == nullptr)
        return;
    // The FOCUSED row only, plus the Selected total (user 2026-07-31): the full per-asset LIST lives
    // on the games list, where you are choosing between games. In here you are ticking one game's
    // parts, so what matters is the part under the cursor and what the ticks add up to.
    std::string text;
...
void GuiMadPageAssetList::act()
{
    if (mRoot == nullptr)
        return;
    // A BACKUP may queue behind a running one, so it must reach the backend; a RESTORE still refuses,
    // because restoring over the live library while something else writes to it is not queueable work.
    if (mRestore && mRoot->busy()) {
