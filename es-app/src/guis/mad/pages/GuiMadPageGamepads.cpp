...
void GuiMadPageGamepadTest::rebuild(const rapidjson::Value& layout)
{
    ...
    if (mProfileKey == "steamdeck")
        footer()->flash(
            ...
            "Hold L+R (6 s) to exit (or Start; auto-stops after ~20 s idle).",
            10000);
    else if (mKind == "wii")
        footer()->flash(
        ...
        footer()->flash(
            "Press each control to light it up. Hold L+R (6 s) to exit (on an 8BitDo "
            "FC30, Start alone powers the pad off — use the L+R shoulders).",
            10000);
}
    ...
    pageRequest("wii.barmode", nullptr,
                [this, alive](bool ok, const rapidjson::Value& payload) {
                    ...
                    mModeLine->setText((mode4 ? "●  " : "○  ") +
                                       ...
                                       MadJson::getString(payload, "explanation"));
                    mModeLine->setColor(mode4 ? MadTheme::color(MadColor::Green) : MadTheme::color(MadColor::Red));
                });
}
