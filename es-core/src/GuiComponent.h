...
// deck-patches TOUCH: pointer event for tap-the-UI touchscreen navigation, dispatched
// by Window::pointerTap()/pointerScroll() through GuiComponent::pointerInput().
struct PointerEvent {
    enum class Type {
        TAP, // A press+release without significant movement.
        SCROLL // A drag (or fling) movement delta.
    };
    Type type;
    glm::vec2 point; // Current pointer position, UI (screen) coordinates.
    // The gesture's origin. SCROLL events are hit-tested with this so a drag stays
    // with the surface it started on even when the finger leaves its bounds.
    glm::vec2 startPoint;
    glm::vec2 delta; // SCROLL: movement since the previous event, UI pixels.
    bool firstEvent; // True for the first SCROLL event of a gesture.
    // True for SCROLL events generated by fling inertia after the finger lifted.
    // Surfaces that need finger-tracked precision (a carried reorder row) ignore
    // these; plain scrolling surfaces treat them like any other SCROLL.
    bool fling {false};

    // The position to hit-test this event with (see startPoint above).
    const glm::vec2& hitPoint() const { return type == Type::SCROLL ? startPoint : point; }
};
...
public:
    ...
    // deck-patches TOUCH: pointer (touchscreen/mouse) input. parentTrans is the same
    // transform the component receives in render(). Returns true when consumed. The
    // default implementation recurses into visible children, topmost (last-rendered)
    // first, without testing its own bounds, so full-screen containers never block
    // their children; leaves and overrides test containment themselves.
    // CONTRACT: overrides that call TouchNavigation::synthesizeInput() must do so as
    // their final action and return immediately, as the synthesized input may delete
    // the dispatching component (e.g. "b" closing the menu). Similarly the default
    // recursion must not touch any member after a child consumed the event.
    virtual bool pointerInput(const PointerEvent& event, const glm::mat4& parentTrans);

    // deck-patches TOUCH: true if the event's hit point lies inside this component's
    // rectangle under trans = parentTrans * getTransform(). Handles translation and
    // scale (menus and lists never rotate; rotated decorative elements are not tap
    // targets).
    const bool pointerWithinBounds(const PointerEvent& event, const glm::mat4& trans) const;
