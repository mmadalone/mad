...
Citron stores DISABLED add-ons (mods/updates/DLC that are unchecked) in the [DisabledAddOns]
...
scalars. Citron reads the array by key NAME (so line order doesn't matter) but iterates exactly
`size` entries, so an edit must maintain the size/index invariants -- we therefore PARSE the whole
section into a model, toggle, and RE-SERIALIZE it (Citron re-normalizes on its next exit anyway).
An add-on whose name is in a title's disabled list = DISABLED; absent = enabled.

Available add-ons for a title are the mod dirs under ~/.local/share/citron/load/<TitleID-HEX>/
(a dir with exefs/romfs/cheats content is one add-on; a dir of sub-option dirs yields
"Mod/SubOption" names, matching Citron's PatchManager). We show the UNION of those and the
persistent disabled list, so a mod that's been disabled but whose files were removed can still be
re-enabled / cleaned up. Rendered per game by GuiMadPageEmuSettings (dynamic bool toggles); Citron
...
def _serialize(model: dict) -> str:
    """The [DisabledAddOns] body for `model` (interleaved; Citron reads by key name)."""
...
@method("citron_addons.get", slow=True)
def _get(params):
    hex_tid = _tid(params)
    text = cfgutil.read_text(_FILE)
    model = _parse(text) if text is not None else {}
    ...
    note = ("Enable/disable mods, updates and DLC for this game (from "
            "~/.local/share/citron/load/<TitleID>/). Off = disabled."
            if rows else
            "No add-ons found for this game. Put mods under "
            "~/.local/share/citron/load/<TitleID-in-hex>/ to see them here.")
    return {"exists": True, "running": _running(), "note": note,
            "groups": [{"title": "Add-Ons", "note": "", "settings": rows}]}


@method("citron_addons.set", slow=True)
def _set(params):
    if _running():
