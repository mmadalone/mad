...
~/.config/citron/input/*.ini. Stored in custom/<TITLEID>.ini's [Controls] as `player_N_profile_name`.

CRITICAL (verified vs citron-neo/emulator): a per-game profile is NOT read from input/<name>.ini at
boot -- Citron BAKES the resolved bindings inline into the per-game ini at save time and reads that
snapshot. So selecting a profile here writes `player_N_profile_name="<name>"` AND copies the
profile's `player_N_button_*`/stick/motion bindings in (each with its \default=false twin). Writing
only the name would drop that player to KEYBOARD at boot. "Use global" removes the player's keys
(absent = inherit the global config). The baked bindings keep the profile's OWN device (guid/port),
which is exactly the intended per-game device PIN that bypasses MAD's pads->players routing (see
memory switch-per-game-profile-routing).

Rendered per game by GuiMadPageEmuSettings (enum rows). Writes refuse while Citron runs.
"""
...
def _bake(body: str, n: int, name: str) -> str:
    ...
    # carry connected/type itself — else Citron boots Players 2-8 disconnected (their default) and
...
@method("citron_pg_input.get", slow=True)
def _get(params):
    tid = _tid(params)
    pg_text = cfgutil.read_text(citron_games.pergame_path(tid)) or ""
    ...
    note = ("Pick a named input profile per player, or 'Use global input configuration' to route "
            ...
            "No input profiles found. Create named profiles in Citron's Controls dialog "
            "(saved under ~/.config/citron/input/) to assign them per game here.")
    return {"exists": True, "running": _running(), "note": note,
            "groups": [{"title": "Input Profiles", "note": "", "settings": rows}]}


@method("citron_pg_input.set", slow=True)
def _set(params):
    if _running():
    ...
    pg = citron_games.pergame_path(tid)
