...
Faithful clone of citron_cheats_cmds.py for Eden: Eden is a Yuzu fork sharing Citron's qt-config.ini
format AND the same on-disk cheats layout, so only the config path, load dir, process name and RPC
namespace differ; the counted-array + build-id logic is identical.

Eden stores DISABLED cheats in [DisabledCheats] of qt-config.ini, the same SimpleIni-faked
...
Cheats for a title live at ~/.local/share/eden/load/<TitleID-HEX>/<mod>/cheats/<BUILDID>.txt as
...
GuiMadPageEmuSettings (dynamic bool toggles); writes refuse while Eden runs.
"""
...
def _resolve_bid(stem: str, config_bids) -> str:
    """The [DisabledCheats] config key for a cheat file whose basename is `stem`. Eden's config
    ...
    cheat for a game Eden has never fingerprinted can't be matched -- an inherent limitation."""
    s = stem.lower()
    prefix = s[:16]
...
def _get(params):
    ...
    note = ("Enable/disable cheats for this game. Off = disabled." if rows else
            ...
            "~/.local/share/eden/load/<TitleID-in-hex>/<mod>/cheats/ to see them here.")
    return {"exists": True, "running": _running(), "note": note,
            "groups": [{"title": "Cheats", "note": "", "settings": rows}]}


@method("eden_cheats.set", slow=True)
def _set(params):
    if _running():
