...
def _append_token(cur: str, token: str) -> str:
    ...
    `"engine:sdl,...,token"` and isn't written unquoted/malformed (Eden's values are quoted)."""
    if cur.endswith('"'):
...
def _apply_stick(text: str, player: str, key: str, value: str) -> str:
    ...
    the rest. Pure text->text (no I/O, no bump) — the buffer stages then flushes it."""
    ...
    if not _player_device(text, player)[0]:
        raise RpcError("EINVAL", f"{_plabel(player)} has no pad here. Configure it in Eden first.")
    if f"axis_{axis}:" not in cur:
        raise RpcError("EINVAL", f"{_plabel(player)}'s stick is unset. Re-set it in Eden.")
...
@method("eden.input_get", slow=True)   # buffered: NO cache=("config",) — the in-memory buffer is truth
...
# InputBuffer and only reach disk on eden.input_save; eden.input_cancel drops them.
# ctx = () because Eden is a single global qt-config.ini; the whole-file working copy
...
def _apply_selector(text: str, edit: dict) -> str:
    ...
    if key == "controller_type":
        ...
        # _CTYPE_VALUES is the standard list; a non-standard value already present is allowed
        # back (preservation) — anything else is rejected (checked below).
    ...
    # Eden ignores a stored value while its `<key>\default` is true — flip it so ours sticks.
...
def _apply(text: str, edit: dict) -> str:
    ...
    other keys survive. Refuses while Eden runs (it rewrites its config on exit)."""
    ...
    if "guid:" not in cur:                       # this one binding is missing/cleared -> re-create it
        cur = f'"engine:sdl,port:{dev_port},guid:{dev_guid}"'   # QUOTED skeleton (Eden values are quoted)
