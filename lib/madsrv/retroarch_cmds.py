"""retroarch.input_* methods -- the RetroArch keybindings (Input) editor.

MAD's RetroArch hub -> Input page configures RetroArch's GLOBAL input bindings in
retroarch.cfg from inside ES-DE (per-player joypad binds, lightgun actions, and the
system hotkeys), so the user never has to drop to desktop mode and open RA's own menu.
RetroArch reads this file at startup and REWRITES THE WHOLE FILE on exit, so a set is
refused while RA is running.

(The old flat "RetroArch" global-defaults page -- retroarch.get / retroarch.set and its
GROUPS descriptor -- was retired in the RA-hub Phase 4 cleanup; global settings now live
in retroarch_settings.py's raset_* categories, per-game/per-system in retroarch_game_cmds.)

Reuses the byte-preserving in-place writer in lib.retroarch_cfg (get_global_option /
set_global_option -- keeps every other line, makes a one-time .mad-bak, atomic
os.replace).
"""
