r"""Shared Yuzu-fork (Citron/Eden) per-game cheats engine (citron_cheats.* / eden_cheats.*).

The fork stores DISABLED cheats in [DisabledCheats] of qt-config.ini, the same
SimpleIni-faked counted array as [DisabledAddOns] but keyed by the game's main-NSO BUILD ID
(a 64-hex string):
...
Cheats for a title live at <load>/<TitleID-HEX>/<mod>/cheats/<BUILDID>.txt as
`[Cheat Name]` bracketed headers; the cheat file BASENAME is the 16-hex (u64) truncation
of the build id, zero-padded to 64 for the config key (so enumeration needs no separate
build-id lookup). A cheat whose name is in its build id's disabled list = DISABLED;
absent = enabled. NOTE: cheats only exist once the user adds cheat files, so this page is
empty until then. Rendered per game by GuiMadPageEmuSettings (dynamic bool toggles);
writes refuse while the emulator runs.

One CheatsEngine instance per fork; constructing it registers <ns>.get/set. The file/load
getters are read PER CALL so a test repointing the shim's _FILE/_LOAD globals is honored
(the shims are citron_cheats_cmds.py / eden_cheats_cmds.py).
"""
...
def _resolve_bid(stem: str, config_bids) -> str:
    """The [DisabledCheats] config key for a cheat file whose basename is `stem`. The
    fork's config stores the FULL build id (20-byte SHA1 = 40 hex) lowercased +
    zero-padded to 64; a cheat file is usually named with only the 16-hex (u64)
    truncation. So: prefer an EXISTING config build_id that shares the file's 16-hex
    prefix (edit the right entry), else zero-pad the stem to 64. The trailing bytes of a
    full build id can't be reconstructed from a 16-hex filename, so a 16-hex cheat for a
    game the fork has never fingerprinted can't be matched -- an inherent limitation."""
    s = stem.lower()
    prefix = s[:16]
...
def _cheats(load: Path, hex_tid: str, config_bids=()) -> dict:
...
class CheatsEngine:
    ...
    def has_content(self, hex_tid: str) -> bool:
        """True if this title has any cheat (a *.txt with cheat headers under
        <load>/<tid>/*/cheats/). Used to hide the empty per-game Cheats tile."""
        return bool(_cheats(self._load(), hex_tid))

    def _do_get(self, params):
        hex_tid = _tid(params)
        text = cfgutil.read_text(self._file())
        disabled = _parse(text) if text is not None else {}
        avail = _cheats(self._load(), hex_tid, disabled.keys())   # resolve against existing entries
        rows = []
        ...
        note = ("Enable/disable cheats for this game. Off = disabled." if rows else
                f"No cheats found. Put a <BuildID>.txt cheat file under "
                f"{self._load_hint}/<TitleID-in-hex>/<mod>/cheats/ to see them here.")
