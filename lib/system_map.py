...
git launchers tree + *.AppImage (in mmadalone/mad). The temp-deck FAN-CONTROL setup is excluded ENTIRELY, and
that includes the innocuous-looking $HOME marker: .config/temp-deck/fan-helper-installed is not a record of
the grant, it is the TRIGGER - deck-post-update.sh reinstalls /etc/sudoers.d/zz-deck-fan (NOPASSWD) and the
root-owned helper non-interactively whenever it exists. Archiving it would let a restore silently grant
passwordless sudo on a Deck that never opted in, and would undo a revocation (--uninstall-fan-helper revokes
by deleting exactly that file). Re-enable deliberately instead. Debris (control-panel .log/.bak, lightgun
.bak variants) is excluded by the tight *.json glob + the exact file lists.
"""
...
_GROUPS = [
    ...
    {"key": "temp-deck", "label": "Temperature monitor",
     "files": ["bin/temp-deck.py"],
     "explain": "The temp-deck system monitor. Fan control is NOT included: re-enable it "
                "deliberately with temp-deck.py --install-fan-helper after a restore."},
]
