...
from lib.openbor_maps import (ABS_ROLE_OVERRIDE, EVDEV_ABS_ROLE,
                              EVDEV_BTN, GEOM_XINPUT, HAPPY_HAT)
from lib.openbor_seating import MAX_PADS, _listed, _node_num, build_plan, class_of
...
# ── the pad plan ─────────────────────────────────────────────────────────────
# The seating rules themselves now live in lib/openbor_seating.py, unchanged, so the MAD
# Preview page can ask the real decider instead of re-deriving it (three Preview bugs came
# out of re-deriving them; see that module's header). Re-exported here so this module's own
# call sites, its log lines and its tests keep reading the same names.
