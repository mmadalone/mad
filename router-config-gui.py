...
class GamepadNav:
    ...
    def _linear(self, cur, pool, fwd):
        """Next/prev focusable in READING order (tree order) — the fallback that makes
        Left/Right traverse EVERY control when there's no spatial neighbour in that
        direction (so the last button in a row advances to the next row, and back)."""
        if cur not in pool:
            return (pool[0] if fwd else pool[-1]) if pool else None
    ...
    def _spatial_move(self, direction: str):
        ...
        if direction in ("up", "down"):
            ...
            # content: spatial-left → linear-prev (so EVERY button is reachable) →
            # cross to the sidebar only from the first content control.
            tgt = (self._nearest(cur, side, "left") if cur_side
                   else (self._nearest(cur, content, "left")
                         or self._linear(cur, content, False)
                         or self._nearest_by_y(cur, side)))
        else:  # right
...
# App
# ---------------------------------------------------------------------------
class App(GamepadTesterMixin, XArcadeTesterMixin, DaphnePageMixin):
    ...
    def _clear(self):
        ...
        if getattr(self, "_dp_capturing", False) or getattr(self, "_dp_proc", None):
            ...
            if _p is not None:
                try:
                    _p.kill()                    # stop the SDL capture subprocess
                except Exception:
                    pass
            self._dp_proc = None
    ...
    def _select_page(self, title, caption, options, on_choose, focus_value=None):
        """options = list of (value, label). Choosing persists via on_choose then returns to the
        previous page. focus_value (optional): pre-select the option whose value == this, so
        reopening the picker remembers the last choice (and scrolls back to it)."""
        def page():
            self._title(title)
            inner = self._scroll()
            if caption:
            ...
            if tgt:
                tgt.focus_set()
                # Re-assert focus once layout has settled so a remembered (scrolled-down) item is
                # also scrolled into view — focus alone in the builder can't scroll an unsized canvas.
                if want is not None:
                    self.root.after(60, lambda w=want: w.winfo_exists() and w.focus_set())
        self.goto(page)
