"""citron_addons.* — the [DisabledAddOns] counted-array editor: parse the (second-pass-ordered)
...
from lib.madsrv import citron_addons_cmds as ac
...
class CitronAddons(unittest.TestCase):
    def setUp(self):
    ...
    def _get(self, tid):
        return rpc._METHODS["citron_addons.get"][0]({"titleid": tid})

    def _set(self, tid, key, value):
        return rpc._METHODS["citron_addons.set"][0]({"titleid": tid, "key": key, "value": value})
