"""citron_cheats.* — enumerate [Cheat Name] headers from load/<TID>/<mod>/cheats/<buildid>.txt,
resolve the build-id key against existing [DisabledCheats] entries (Citron stores a full 40-hex
...
class CitronCheats(unittest.TestCase):
    def setUp(self):
    ...
    def _rows(self, tid=_TID):
        return {s["label"]: s for s in
                rpc._METHODS["citron_cheats.get"][0]({"titleid": tid})["groups"][0]["settings"]}

    def _set(self, key, value, tid=_TID):
        return rpc._METHODS["citron_cheats.set"][0]({"titleid": tid, "key": key, "value": value})

    def test_registered(self):
        self.assertIn("citron_cheats.get", rpc._METHODS)
        self.assertIn("citron_cheats.set", rpc._METHODS)
    ...
    def test_empty_state(self):
        p = rpc._METHODS["citron_cheats.get"][0]({"titleid": "0100152000022000"})
