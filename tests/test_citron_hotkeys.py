"""citron_hk.* — the format-adaptive Hotkeys remapper: enumerate actions from the live nested
...
from lib.madsrv import citron_hotkeys_cmds as hk
...
class CitronHotkeys(unittest.TestCase):
    def setUp(self):
    ...
    def _call(self, verb, **params):
        return rpc._METHODS[f"citron_hk.{verb}"][0](params)
    ...
    def test_registered(self):
        for v in ("input_get", "input_set", "input_clear", "input_save", "input_cancel"):
            self.assertIn(f"citron_hk.{v}", rpc._METHODS)
    ...
    def test_flush_replays_onto_fresh_disk_preserving_foreign_edits(self):
        # LANDMINE: citron.* rewrites [Controls]/[System] in the SAME qt-config.ini. A hotkey save
        ...
        # a concurrent writer (citron.*) commits a [Controls] change AFTER we staged
