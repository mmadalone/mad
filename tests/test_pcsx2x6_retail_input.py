...
class SinglePort(unittest.TestCase):
    ...
    def test_set_targets_the_pinned_port(self):
        ...
        self._with(ini, lambda: set_usb2(
            {"id": "guncon2-retail_Trigger", "kind": "gun", "gun_kind": "mouse", "value": "1"}))
    ...
    def test_selector_set_scoped_to_the_pinned_gun(self):
        ...
        self._with(ini, lambda: sel_usb1({"key": "cursor_scale", "value": "0.12"}))
    ...
    def test_clear_unbinds_on_the_pinned_port(self):
        ...
        self._with(ini, lambda: set_usb1(
            {"id": "guncon2-retail_Start", "kind": "gun", "gun_kind": "key", "value": "enter"}))
        self.assertIn("guncon2-retail_Start", inifile.section_body(ini.read_text(), "USB1"))
        self._with(ini, lambda: clr_usb1({"id": "guncon2-retail_Start"}))
    ...
    def test_write_refused_while_emulator_running(self):
        # pcsx2x6 rewrites its ini on exit, so a live-emulator write must refuse (EBUSY) and change
    ...
    def test_write_bumps_config_rev(self):
        # a successful write MUST bump staterev "config" or MAD keeps serving the stale page.
        ini = self._ini()
        set_usb1 = rpc._METHODS["x6r_usb1.input_set"][0]
